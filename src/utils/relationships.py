...
def grad_fn_map(grad_fn: AutogradFunction) -> Type:
    ...
    if type(grad_fn) is type(torch.sum(TA).grad_fn.next_functions[0][0]):
        XAF_class = XAF.accumulation.AccumulateGradX
