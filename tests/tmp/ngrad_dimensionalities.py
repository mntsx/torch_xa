...
def test_3() -> None:
    ...
    T0 = torch.rand(size=(1, 6), requires_grad=True)
    T1 = torch.rand(size=(4, 6), requires_grad=True)
    O = torch.add(T0, T1)
    backward(source=O, order=2)
    assert T0.ngrad[0].shape == (O.numel(), T0.numel())
    ...
    T0 = torch.rand(size=(4, 1), requires_grad=True)
    T1 = torch.rand(size=(4, 6), requires_grad=True)
    O = torch.add(T0, T1)
    backward(source=O, order=2)
    assert T0.ngrad[0].shape == (O.numel(), T0.numel())
...
def test_5() -> None:
    ...
    T0 = torch.rand(size=(4, 8), requires_grad=True)
    T1 = torch.rand(size=(4, 6), requires_grad=True)
    ...
    assert T0.ngrad[0].shape == (O.numel(), T0.numel())
    assert T1.ngrad[0].shape == (O.numel(), T1.numel())
    assert T2.ngrad[0].shape == (O.numel(), T2.numel())
    ...
    T0 = torch.rand(size=(1,), requires_grad=True)
    T1 = torch.rand(size=(4, 6), requires_grad=True)
    T2 = torch.rand(size=(6, 8), requires_grad=True)
    O = torch.addmm(input=T0, mat1=T1, mat2=T2)
    backward(source=O, order=2)
...
def test_6() -> None:
    ...
    backward(source=O, order=2)
    assert T.ngrad[0].shape == (O.numel(), T.numel())
...
def test_11() -> None:
    ...
    T: Tensor = torch.rand(size=(2, 3, 4, 5), requires_grad=True)
